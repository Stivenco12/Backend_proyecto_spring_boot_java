...
public class PaymentTypeController {
    ...
    public ResponseEntity<?> view(@PathVariable Long id) {
        Optional<Payment_type> optional = paymentTypeService.findById(id);
    ...
    @PostMapping
    public ResponseEntity<?> create(@Valid @RequestBody Payment_type paymentType, BindingResult result) {
        if (result.hasErrors()) {
            return validation(result);
        }
        return ResponseEntity.status(HttpStatus.CREATED).body(paymentTypeService.save(paymentType));
    }

    @PutMapping("/{id}")
    public ResponseEntity<?> update(@RequestBody Payment_type paymentType, @PathVariable Long id) {
        Optional<Payment_type> optional = paymentTypeService.update(id, paymentType);
        if (optional.isPresent()) {
    ...
    public ResponseEntity<?> delete(@PathVariable Long id) {
        Optional<Payment_type> optional = paymentTypeService.delete(id);
        if (optional.isPresent()) {
            return ResponseEntity.ok(optional.orElseThrow());
        }
        return ResponseEntity.notFound().build();
