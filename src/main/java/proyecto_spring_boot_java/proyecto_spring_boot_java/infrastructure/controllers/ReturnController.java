...
public class ReturnController {
    ...
    public ResponseEntity<?> view(@PathVariable Long id) {
        Optional<Return> optional = returnService.findById(id);
    ...
    @PostMapping
    public ResponseEntity<?> create(@Valid @RequestBody Return ret, BindingResult result) {
        if (result.hasErrors()) {
            return validation(result);
        }
        return ResponseEntity.status(HttpStatus.CREATED).body(returnService.save(ret));
    }

    @PutMapping("/{id}")
    public ResponseEntity<?> update(@RequestBody Return ret, @PathVariable Long id) {
        Optional<Return> optional = returnService.update(id, ret);
        if (optional.isPresent()) {
    ...
    public ResponseEntity<?> delete(@PathVariable Long id) {
        Optional<Return> optional = returnService.delete(id);
        if (optional.isPresent()) {
            return ResponseEntity.ok(optional.orElseThrow());
        }
        return ResponseEntity.notFound().build();
